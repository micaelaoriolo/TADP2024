...
trait Estado {
    ...
    def realizaActividad(actividad: Actividad): Estado = actividad(this).map(_.intentarEvolucionar())
}

object Estado {
    def unapply(estado: Estado): Option[Pokemon] = Some(estado.pokemon) //un estado deconstruido es un pokemon 
}
